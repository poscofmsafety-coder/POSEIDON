...
## V6.23 상단 버튼 가독성 개선
- 홈/현장 QR/AI 관제 운영 중/관리자 텍스트 줄바꿈 방지
- 날짜·시간 숫자 세로 분리 방지
- 좁은 데스크톱에서는 버튼을 찌그러뜨리지 않고 상단 액션 영역 전체를 다음 행으로 배치
- 모바일 아이콘 모드는 기존 동작 유지


## V6.24 상단바 위치 통일

장치 관리 등 페이지별 콘텐츠 폭과 관계없이 관리자 상단 액션 버튼 위치를 동일하게 고정했습니다.

## V6.25 Safety Don 상단 간격 보완
...
## V6.31 Smart Safety Ball 앱형 모니터 UI
...
## V6.32 로그인/로그아웃 가독성 개선
- 초기 로그인 제목을 `POSEIDON`과 `스마트 안전지킴이` 두 줄로 분리
- 우측 상단 로그아웃 버튼에 `로그아웃` 텍스트 추가 및 반응형 여백 확보
